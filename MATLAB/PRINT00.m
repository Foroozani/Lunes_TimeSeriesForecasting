...
D=cell(sizet,nlay);
% data reading loop
% matrix: data(layer,timeperiod)
for layer=1:nlay
    for period=1:sizet
        f=fullfile(cart,listD{(layer-1)*sizet+period});
        Fi=last2(f,2);
        WSdata{layer,period}=Fi(:,2);
        DdataNcw{layer,period}=Fi(:,1);
        % components calculation
        Ucdata{layer,period}=WSdata{layer,period}.*(-sind(DdataNcw{layer,period}));
        Vcdata{layer,period}=WSdata{layer,period}.*(-cosd(DdataNcw{layer,period}));
    end
end

%% GENERAL DATA SAVING
save data.mat cart x y z Xl Yl Udata Vdata Wdata Tdata MHdata WSdata DdataNcw zlayc
...
for period=1:frogt:tper
    ...
    axis([Xmin Xmax Ymin Ymax 0 2000]);
    % set background
